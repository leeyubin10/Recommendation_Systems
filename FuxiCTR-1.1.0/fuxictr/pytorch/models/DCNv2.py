...
class DCNv2(BaseModel):
    ...
    def forward(self, inputs):
        ...
        # Transformer 부분
        transformer_out = self.transformer(flat_feature_emb.unsqueeze(1))  # Transformer expects 3D input (seq_len, batch_size, features)
        transformer_out = transformer_out.squeeze(1)  # (batch_size, features)
        ...
        if self.model_structure == "crossnet_only":
            ...
            final_out = torch.cat([cross_out, transformer_out, dnn_out], dim=-1)  # CrossNet, Transformer, DNN 병합
