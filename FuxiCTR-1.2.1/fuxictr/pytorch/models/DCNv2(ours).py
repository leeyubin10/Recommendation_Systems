...
class DCNv2(BaseModel):
    def __init__(self, 
                 ...
                 learning_rate=1e-3, 
                 embedding_dim=10, 
                 stacked_dnn_hidden_units=[128, 64],  # 더 깊은 네트워크 예시
                 parallel_dnn_hidden_units=[256, 128],  # 더 복잡한 네트워크 예시
                 dnn_activations="ReLU",
                 num_cross_layers=3, 
                 net_dropout=0.5,  # Dropout 비율 증가
        ...
        if self.model_structure in ["stacked", "stacked_parallel"]:
            self.stacked_dnn = MLP_Layer(input_dim=input_dim,
                                         ...
                                         batch_norm=batch_norm,  # 배치 정규화 적용
                                         use_bias=True)
